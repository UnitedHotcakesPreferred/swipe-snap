...
var	setDelay, snapLength,
	...
	var	newFocus =	arguments[0] - (arguments[1] && portArea.find(liPlate(arguments[0])).hasClass('select') ? 0 : 1),
		...
		if (newSnap != (isVert ? portArea.scrollTop() : portArea.scrollLeft())) {
			pullTo(newFocus+1, newSnap, (portArea.find(liPlate(newFocus)).hasClass('select') && portArea.find(liPlate(newFocus)).hasClass('swipe_out') ? 750 : 0));
		}
	},
	newDisplay = function() {	// on scroll
	...
	var	endTug, startTug, startFirst, pageFrac, toSelect,
		...
		if (newSnap != scrollFrom) setDelay = self.setTimeout(function() { pullTo(count, newSnap, 99); }, 300);
	...
	var	toc = jQuery('ul.toc'),
		...
		var	imgHref = arguments[0];
			...
			if (fitWidth || fitHeight) imgHref.children('img').attr(isVert ? 'height' : 'width', snapLength);
		};
		if (fitHeight) {
			imgHeight = portHeight - (borderGap + 2 * padHeight);
			imgArea.attr('style', 'max-height: ' + px(imgHeight));
			imgWidth = Math.round(imgArea.width());
		...
		if (fitWidth) {
			...
			imgHeight = Math.round(imgArea.height());
		}
		snapLength = (isVert) ? imgHeight : imgWidth;
		liShow.find('a').css({'width': px(imgWidth), 'height': px(imgHeight)});
		...
		for (var ix=1;ix<=pageTotal;ix++) {
			...
			var	isShow, addTOC,
				...
				addTOC = toc.children('li:last-child');
				addTOC.addClass(idPlate(ix));
				addTOC.append(document.createElement('a'));
				addTOC.children('a').text((ix > 9 ? '' : '0') + ix.toString()).attr('href', isPage).on('click', function() {
	...
	//	hold place
		...
		portArea.find('img').attr(isVert ? 'height' : 'width', '');
		portArea.find('img, a, strong').attr('style', '');
		portArea.attr('style', '');
